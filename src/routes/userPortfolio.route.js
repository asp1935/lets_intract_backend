...
import { addPortfolioClient, addPortfolioGallery, addPortfolioServices, deletePortfolioItem, getPortfolio, getUserPortfolio, createUserPortfolio, updateUserPortfolio, deletePortfolio, updateUserProfile, updateIncludeLink,   upsertPaymentDetails, deletePaymentDetails } from "../controllers/userPortfolio.controller.js";
...
router.route('/add-payment/:id').patch(verifyJWT, authorize(['portfolio']), portfolioUpload.single('qrFile'), upsertPaymentDetails);
// router.route('/update-payment/:id').patch(verifyJWT, authorize(['portfolio']), updatePaymentDetails);
router.route('/delete-details/:id').delete(verifyJWT, authorize(['portfolio']), deletePaymentDetails);
